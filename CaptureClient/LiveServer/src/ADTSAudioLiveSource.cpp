...
ADTSAudioLiveSource::createNew(UsageEnvironment& env, Buffer* buf) {
  ...
  do {
    ...
    if (buf->read(fixedHeader, sizeof fixedHeader ) < sizeof fixedHeader) break;

    // Check the 'syncword':
    if (!(fixedHeader[0] == 0xFF && (fixedHeader[1]&0xF0) == 0xF0)) {
      env.setResultMsg("Bad 'syncword' at start of ADTS Stream");
      break;
    }
...
#endif
    return new ADTSAudioLiveSource(env,buf, profile,
...
void ADTSAudioLiveSource::doGetNextFrame() {
  ...
  int numBytesRead = data->read(fTo, numBytesToRead);
